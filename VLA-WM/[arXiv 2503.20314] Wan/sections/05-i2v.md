...
### 🎯 Figure 18 完整解读 —— 把所有学过的概念串起来

> 这一张图调动了所有学过的核心概念：[VAE](../../../_concepts/vae.md) / [CLIP](../../../_concepts/clip.md) / [Encoder-Decoder](../../../_concepts/encoder-decoder.md) / [Diffusion 基础](../../../_concepts/diffusion-basics.md) / [Block 堆叠](../../../_concepts/transformer-block-stacking.md) / [Normalization](../../../_concepts/normalization.md) / [Pre/Post-training](../../../_concepts/pretraining-vs-posttraining.md) / [训练 vs 推理](../../../_concepts/training-vs-inference.md) / [PSNR](../../../_concepts/psnr.md)。

#### 概览：5 大组件 + 3 路信号

```
                  输入: First frame (图) + Text prompt
                                  │
        ┌─────────────────────────┼─────────────────────────┐
        ▼ 路径 A: VAE 路            ▼ 路径 B: CLIP 路           ▼ 路径 C: 文本路
   保留空间细节                  抽全局语义                   编码文字意图
        │                         │                          │
        └────────────────────────┬┴──────────────────────────┘
                                  ▼
...
"A woman in a white spin is doing yoga on a soccer field"
            ...
            umT5 Encoder（多语言双向）
                  │
                  ▼
       T-Tokens (一系列文本 token 向量)
```

#### 第 2 步：图像的双路处理（I2V 关键）

**路径 A · VAE 路（保留空间细节）**：
```
First frame + 0-padding frames → Wan-Encoder → condition latent (16 × t × h × w)
                                                          │
                                          沿 channel concat 进 DiT 输入
```

**路径 B · CLIP 路（抽全局语义）**：
```
First frame → CLIP Image Encoder → ~768 维向量 → 3-layer MLP → 全局上下文
                                                                    │
                                                        通过 decoupled cross-attn 注入 DiT
```

⚠️ **同一张图过两个 encoder ≠ 冗余**：VAE 给"图怎么长的"，CLIP 给"图是什么"。

#### 第 3 步：Mask 任务路由

```
Mask: [1, 0, 0, ..., 0]  ← 首帧 1（已知），其他 0（待生成）
```

**同一模型 + 不同 mask = 不同任务**（I2V / 续写 / 首尾帧 / 帧插值）。

#### 第 4 步：Diffusion Process 内部 —— N × DiT Blocks

```
   x_t (noise latent)        Timestep t
        │                        │
        ▼                        ▼
   + condition latent + mask  共享 MLP → 6 modulation params
   (channel concat)
        │
        ▼
╔═══════════════════════════════════╗
║   Block 1（每 block 同结构异参数） ║
║   ─────────────────────────────  ║
║   LayerNorm (RMSNorm) ← α₁, β₁    ║
║   Self-Attention (full ST attn)   ║
║   LayerNorm ← α₂, β₂              ║
║   Cross-Attention 1 ← umT5 文本    ║
║   Cross-Attention 2 ← CLIP 全局   ║   ← decoupled (I2V 比 T2V 多的)
║   LayerNorm ← α₃, β₃              ║
║   FFN                             ║
╚═══════════════════════════════════╝
        │
...
⚠️ **I2V 比 T2V 多一路 cross-attention**：除了 umT5 文本，还有 CLIP 全局。两路平行不干扰（"decoupled"）。

#### 第 5 步：解码出视频

```
generated latent → Wan-Decoder → 输出视频 (T × H × W × 3)
```

---

#### 完整 Forward Trace（推理时一次完整生成）

```
推理 = DiT 循环 50 次：

t=0：纯噪声开始
   x_0 ~ N(0, I)，首帧位置已被锚定为 VAE encode(用户图)
   ↓ DiT forward 1: pred_v_1 = DiT(x_0, T-tokens, CLIP, mask, timestep=0.02)
   ↓                  x_t1 = x_0 + step × pred_v_1
   ↓ DiT forward 2: ...
   ↓ ... 重复 50 次 ...
   ↓ DiT forward 50: 最后一步
t=1：清晰的 latent video
   ↓ Wan-Decoder
   ↓
最终 RGB 视频
```

每次 forward 的 **(T-tokens, CLIP context, mask) 不变**，变的只是 x_t 和 timestep。

#### 训练 vs 推理 关键差异

| | 训练 | 推理 |
|---|---|---|
| **CLIP encoder 是否启用** | Pre-training 阶段**关闭** / SFT 阶段才开启 | 始终启用 |
| **DiT forward 次数** | 1 次/样本 | **50 次/视频** |
| **首帧** | 训练数据自身的首帧 | 用户上传图（VAE encode） |
| **梯度** | 反向传播更新参数 | 冻结，无梯度 |

#### 🪄 一句话理解 Figure 18

> Wan-I2V = T2V + 两个图像旁路（VAE 局部细节 + CLIP 全局语义）+ Mask 任务路由。**3 路信号汇聚到 DiT，N 次 block 处理，50 次 timestep 迭代，最后 VAE 解码成视频**。
