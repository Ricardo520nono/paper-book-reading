...
| 工作 | 做了 ✅ | 没做到 ❌ | 链接 | 备注 |
|---|---|---|---|---|
| **Ctrl-World (2025/10)** | 提出 frame-level cross-attention 注入 action (Cartesian 6D pose + 1D gripper); 多视角联合预测 (3 cam, 含 wrist-view) + pose-conditioned memory 防漂移; 验证 WM 作为 policy ranker: 3 个 SOTA VLA × 7 task, 排名对齐真机 (y=0.87x-0.04); 验证 WM 作为合成数据源: π0.5 在 novel object/instruction 上 +44.7% | 训练数据仅 DROID 真机遥操作 (76k success + 19k failure 但仍在 human-attempted 范围内); 测的 3 个 policy 都是 expert-quality VLA, 没测 sub-optimal / 早期 RL checkpoint 的 policy; 没直接喂 off-expert action (counterfactual / random-feasible / adversarial) 评估 WM; 合成数据只筛 successful trajectory, 跳过 failure 分布 | [📄 summary](../%5BarXiv%202510.10125%5D%20Ctrl-World/sections/summary.md) | 翔哥已点名 · a 类 (也提供 b 类 SVD → AC-WM finetune pipeline) |
| **Dreamer 4 (2025/09)** | AC-WM (Causal Tokenizer + 2D 时空 Transformer) + Agent (policy/value head) 联合系统; action 注入方式: action 作为 token 与 latent / time / step-size token 拼接喂入 dynamics (≠ cross-attention); 3-Phase pipeline: unlabeled video pretrain → labeled finetune → imagination RL; shortcut forcing 目标蒸馏 flow matching (64 → 4 步); 首次纯 offline 在 Minecraft 拿到 diamond, 比 OpenAI VPT 用 100× 更少数据 | 场景是 Minecraft game, 不是 robot manipulation; Action 是 121 类离散键鼠 + binary, 不是数值 robot action; 训练数据来自 contractor gameplay (human expert demo), 未覆盖 off-expert action 分布; 无 off-expert / OOD action 评估设计 | [📄 summary](../%5BarXiv%202509.24527%5D%20Dreamer%204/sections/summary.md) | Hafner 团队 · DeepMind · 场景偏离 (Minecraft) |
| **AdaWorld (2025/03)** | AC-WM (autoregressive video gen + latent action conditioning); action 注入方式: 从 video 帧间用 information bottleneck autoencoder unsupervised 抽 latent action, 作为 condition 输入 WM (不需 action label); Latent Action Autoencoder + Action-Aware Pretraining 两步范式; 跨 Habitat / Minecraft / DMLab / nuScenes 4 个 unseen 环境通用; Few-shot adaptation: 800 步 / 30 秒 / 单 GPU 适配新环境 | 场景是 game / 导航 / 驾驶 mixed, 不是 robot manipulation; Action 是 learned latent vector (连续, unsupervised), 不是数值 robot action; 训练数据是 unlabeled video, 没有显式 action 分布概念, 不存在 off-expert 测试设计; 关注跨环境泛化 (visual / dynamics OOD), 不关注 OOD action | [📄 summary](../%5BarXiv%202503.18938%5D%20AdaWorld/sections/summary.md) | latent action 路线 · 不需 action label |
| **WorldGym / WPE (2025/06)** | AC-WM (autoregressive diffusion video gen); action 注入方式: chunk-wise bidirectional attention + causal cross-chunk (16 帧 chunk); 用 VLM (GPT-4o) 作 reward model 自动判 success; 测 3 个 VLA (RT-1-X / Octo / OpenVLA) 在 Bridge / Open X-Embodiment 上, 真机 vs WM 成功率相关性 r=0.78; 能改 image / 改 language 测 OOD generalization | 没考虑 OOD action (OOD 维度只在 image 和 language); 训练数据是真机 expert demo (Bridge), 未覆盖 off-expert action; 方法贡献偏弱, 主要是 evaluation framework 整合 | [📄 summary](../%5BarXiv%202506.00613%5D%20WPE/sections/summary.md) | 可能与翔哥已填 "WorldGym (2025/05)" 是同一篇 · Ctrl-World §5.2 baseline |
| Motus | _(待填，跳过 - 翔哥已知)_ | _(跳过)_ | arXiv 2512.13030 | 梦飞已找到 · 翔哥已知，跳过 |
